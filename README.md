...
| 人工审批 | 可选在「方案确定后 / 最终交付前」暂停等你点头，超时 10 分钟自动否决；服务重启后未决审批保留并引导断点重跑 |
| 可靠提交 | 每次执行携带唯一提交 ID：网络闪断自动重试，响应丢失后重发不重复执行（幂等去重，覆盖聊天/任务/单聊/重跑/终端/卡牌） |
| 成果清单 | 会话产出自动登记成果清单（ARTIFACTS.md），下游智能体按需读取具体文件，上下文体积与成果数量解耦 |
